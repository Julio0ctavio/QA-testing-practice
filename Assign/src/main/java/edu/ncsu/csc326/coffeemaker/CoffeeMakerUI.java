...
public class CoffeeMakerUI {
	...
	public enum Mode {WAITING, ADD_RECIPE, DELETE_RECIPE, EDIT_RECIPE, ADD_INVENTORY, CHECK_INVENTORY, PURCHASE_BEVERAGE, INSERT_MONEY};
	...
	public void UI_Input(Command cmd) {
		...
		case INSERT_MONEY :
			if (cmd instanceof InsertMoney) {
				InsertMoney insCmd = (InsertMoney)cmd;
				moneyInserted = insCmd.amount;
				status = Status.OK;
				mode = Mode.WAITING;
			} else {
				defaultCommands(cmd);
